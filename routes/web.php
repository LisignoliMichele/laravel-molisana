...
Route::get('/', function () {
    ...
    foreach($data as $key => $pasta){
        ...
        // aggiungiamo la key 'id' per identificare ogni prodotto numerandolo con la posizione nell'areray data
        $pasta['id'] = $key;
 
        // dividiamo le varie paste per lunghezza in array
        if ($pasta["tipo"] == "lunga"){
            $lunga[] = $pasta;
    ...
    // ritorniamo tutti i dati che serviranno a costruire la pagina
    return view('home',[
        "pastalunga" => $lunga,
        "pastacorta" => $corta,
...
Route::get('notizie-recenti', function () {
    return view('news');
})->name('news');
